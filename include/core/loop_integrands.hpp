...
class triangle_integrand
{
    ...
    {
        complex<double> D = x1*(_ima2 - XI*sqrt(_ima2)*_wa) 
...
class box_integrand
{
    ...
    // Get masses squared from the parent triangle function
    inline void update_masses(array<double,2> st, array<double,4> ex_m2, array<double,4> in_m2, array<double,4> in_w)
    {
    ...
    // Evaluate the integrand at fixed values of the feynman parameters
    inline complex<double> eval(double x0, double x1, double x2, double x3)
    {
