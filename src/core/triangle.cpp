...
namespace hadMolee
{
    ...
    {
        complex imA = _imA - XI*_wA/2.;
        complex imB = _imB - XI*_wB/2.;
        complex imC = _imC - XI*_wC/2.;
        ...
        complex q_C = sqrt((_emA2 + IEPS)-pow(_emB+_emC, 2.))*sqrt((_emA2 - IEPS)-pow(_emB-_emC, 2.))/(2.*_emA);
        complex E_C = sqrt(q_C*q_C + _emC2);

        complex a = pow(q_C*mu_AC/_imA, 2.);

        complex b_AC = imA + imC + E_C - _emA;
        complex b_BC = imB + imC       - _emA;
        ...
        complex cut_1 = atan( (c_2 - c_1)        / sqrt( 4.*a*(c_1     - XI * _eps) ));
        complex cut_2 = atan( (c_2 - c_1 - 2.*a) / sqrt( 4.*a*(c_2 - a - XI * _eps) ));
